...
def summary(infolder, outfolder, includestartend=False, recodefile=None):
    ...
    summary['numberofdays'] = alldata[['dur','participant_id']].groupby('participant_id').agg(['count'])
    ...
    summary[customcols].to_csv(os.path.join(outfolder,'summary_daily_appcoding.csv'))
